...
One-off guest invitations do not necessarily require a partner entry. However, even for smaller relationships, adding a partner gives you [cross-tenant access policy](/docs/concepts/cross-tenant-policies) control and [trust score](/docs/concepts/trust-score) tracking — both of which are valuable if the relationship grows.
...
Categories are used for filtering on the [partner list](/docs/partners/viewing-partners) and in reporting. Consistent categorization across your team makes it easier to apply policies uniformly and identify patterns.
...
- **Vendors** — Limit [B2B collaboration](/docs/glossary/glossary) to specific applications relevant to the engagement. Avoid enabling B2B Direct Connect unless there is a clear need for shared channels. Do not trust MFA unless you have verified the vendor's MFA enforcement.
...
> **Good to know:** Use [templates](/docs/admin/templates) to codify these recommendations. A well-designed set of templates (e.g., "Vendor - Restricted", "Subsidiary - Trusted") ensures consistency and reduces the chance of misconfiguration.
...
- **Quarterly** — Run [access reviews](/docs/access-reviews/overview) for guest users from each partner. Confirm that active guests still need access and remove those who do not.
...
Sorting the partner list by trust score (ascending) is the fastest way to identify partners that need attention. See [Viewing Partners](/docs/partners/viewing-partners) for sorting instructions.
...
1. **Review guest users** — Navigate to the partner's [detail page](/docs/partners/partner-details) and review the guest user list. Determine which guests should be removed.
